...
<?php
	...
	if(mysql_num_rows($result) >= 1 ) {
		...
		$pdfito = new FPDF("L");
		$pdfito->AddPage();
		$pdfito->Image("http://localhost/ssmanager/img/cosfa.png", 10, 10, 25, 25, 'png');
		...
		$pdfito->Cell(100, 20, utf8_decode("Observación"), 1, 0, 'C');
		$pdfito->Cell(0, 20, utf8_decode("Firma"), 1, 0, 'C');
		$pdfito->Ln(20);
		$offset = 0;
		while($rows = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$pdfito->Cell(100, 20, utf8_decode("{$rows['observations']}"), 1, 0, 'C');
			$pdfito->Cell(0, 20, utf8_decode(""), 1, 0, 'C');
			if(file_exists("img/students/signs/{$rows['observationid']}.jpg")) {
				$pdfito->Image("http://localhost/ssmanager/img/students/signs/{$rows['observationid']}.jpg", 252, 113 + $offset, 32, 16, 'jpeg');
				$offset += 20;
			}
			else {
				$pdfito->Cell(0, 120, utf8_decode("Firma no definida"), 1, 0, 'C');
			}
			$pdfito->Ln(20);
		}
